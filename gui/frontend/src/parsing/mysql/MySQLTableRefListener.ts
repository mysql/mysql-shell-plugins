...
export class MySQLTableRefListener implements MySQLParserListener {
    ...
    public exitTableRef = (ctx: TableRefContext): void => {
        ...
        if (!this.fromClauseMode || this.level === 0) {
            if (ctx.qualifiedIdentifier()) {
                let table = unquote(ctx.qualifiedIdentifier()!.identifier().text);
                let schema = "";
                if (ctx.qualifiedIdentifier()?.dotIdentifier()) {
                    // Full schema.table reference.
                    schema = table;
                    table = unquote(ctx.qualifiedIdentifier()!.dotIdentifier()!.identifier().text);
                }
                ...
                this.context.pushTableReference({
                    schema: "",
                    table: unquote(ctx.dotIdentifier()!.identifier().text),
                    alias: "",
                });
            }
        }
    ...
    public exitTableAlias = (ctx: TableAliasContext): void => {
        ...
        if (this.level === 0) {
            ...
            this.context.setLastAlias(unquote(ctx.identifier().text));
        }
    };
