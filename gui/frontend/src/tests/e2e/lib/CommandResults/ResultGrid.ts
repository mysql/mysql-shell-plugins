 ...
 */
export class ResultGrid extends Result {

    /** The columns of the result query, if exists*/
    public columnsMap: Map<string, string> | undefined;
    ...
    public setColumnsMap = async (): Promise<void> => {
        ...
        }, constants.wait5seconds, `Could not set the columns map`);
    };
    ...
    private getCell = async (gridRow: number, gridColumn: string): Promise<WebElement> => {
        ...
        await driver.wait(async () => {
            try {
                const result = await new E2ECodeEditor(this.id).getResult(this.command, this.id);
                this.setResultContext(result);

                const resultGrid = this.resultContext;
