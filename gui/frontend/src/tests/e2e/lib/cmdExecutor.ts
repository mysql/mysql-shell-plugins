...
export class CommandExecutor {
     ...
     * @returns A promise resolving when the mouse cursor is placed at the desired spot
     */
    private getResult = async (driver: WebDriver, cmd?: string, searchOnExistingId?: string):
        Promise<WebElement> => {
        ...
        await driver.wait(async () => {
            try {
                ...
                if (isTabResult) {
                    ...
                    resultContent = await result.findElement(locator.notebook.codeEditor.editor.result.text.exists);
                } else if (isOutput) {
                    return true;
                }
     ...
     * @returns A promise resolving with the truthiness of the function
     */
    private expectResultTabs = (cmd: string | undefined): boolean => {
        if (cmd) {
