...
def add_data(caption, content, data_category_id, tree_identifier, folder_path=None, profile_id=None, be_session=None):
    ...
    if caption.strip() == "":
        raise MSGException(Error.CORE_INVALID_PARAMETER,
                           f"Parameter 'caption' cannot be empty.")
    ...
    with BackendDatabase(be_session) as db:
        with BackendTransaction(db):
            ...
            if max_privilege <= read_only:
                ...
                backend.add_data_to_folder(
                    db, id, folder_user_group_id, read_only)
            else:
                raise MSGException(Error.MODULES_SHARING_WITH_HIGHER_PERMISSIONS,
        ...
        with BackendTransaction(db):
            ...
            if backend.get_profile_owner(db, profile_id) == context.web_handler.session_user_id:
                if max_privilege <= read_only:
                    ...
                    backend.add_data_to_folder(
                        db, id, folder_profile_id, read_only)
                else:
                    raise MSGException(Error.MODULES_SHARING_WITH_HIGHER_PERMISSIONS,
                                       "Cannot assign data to profile with higher permission than user has.")
            else:
                raise MSGException(Error.MODULES_USER_HAVE_NO_PRIVILEGES,
...
def add_data_category(name, parent_category_id=None, be_session=None):
    ...
    with BackendDatabase(be_session) as db:
        ...
        if search:
            raise MSGException(Error.MODULES_INVALID_DATA_CATEGORY,
                               "Data category already exists.")
        else:
            res = db.execute(
                """SELECT MAX(id) FROM data_category""").fetch_one()
...
def move_data(id, tree_identifier, linked_to, link_id, source_path, target_path, be_session=None):
    ...
    with BackendDatabase(be_session) as db:
        with BackendTransaction(db):
            ...
            source_folder_id = backend.get_folder_id(
                db, root_folder_id, source_path)
            if source_folder_id is None:
                raise MSGException(Error.CORE_INVALID_PARAMETER,
                                   f"Cannot find the given 'source_path'.")
            target_folder_id = backend.get_folder_id(
                db, root_folder_id, target_path)
