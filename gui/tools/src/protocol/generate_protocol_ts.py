...
class ProtocolBuilder:
    ...
    def build(self) -> None:
        ...
        handle_type_hints = self._plugin_name not in NON_ANNOTATED_PLUGINS
    ...
    def _generate_parameter_bindings(self, bindings: dict, nullables: dict) -> str:
    ...
    def _generate(self) -> None:
        ...
        all_interfaces: str = self._generate_parameter_bindings(
        ...
        if self._plugin_name not in NON_ANNOTATED_PLUGINS:
            if self._plugin_name in type_hint_interfaces:
                plugin_definitions = type_hint_interfaces[self._plugin_name].generate(
                )
                all_interfaces += '\n'.join(plugin_definitions) + "\n"

        self._protocol_file_content = self._protocol_file_content.replace(
            "{__AUTOGENERATED_PARAMETER_TYPES__}", all_interfaces)
