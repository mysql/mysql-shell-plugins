...
export class MyServiceSakilaActorRequest extends MyServiceSakilaObjectRequest {
    ...
    public findMany = async (args?: IFindOptions<IMyServiceSakilaActorParams>): Promise<IMrsResultList<IMyServiceSakilaActor>> => {
        ...
        const response = await request.where(args?.where).orderBy(args?.orderBy).limit(args?.take).offset(args?.skip).fetchAll();

        return response;
    };

    public findFirst = async (args?: IFindOptions<IMyServiceSakilaActorParams>): Promise<IMyServiceSakilaActor | undefined> => {
        const request = new MrsBaseObjectQuery<IMyServiceSakilaActor, IMyServiceSakilaActorParams, MyServiceSakilaActor>(
            this.schema, MyServiceSakilaActor, MyServiceSakilaActor.requestPath, args?.select);
