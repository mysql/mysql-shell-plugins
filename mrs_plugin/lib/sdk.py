...
def get_base_classes(sdk_language, prepare_for_runtime=False):
    if (sdk_language == "TypeScript"):
        ...
        raise Exception(f"The SDK language {sdk_language} is not supported yet.")
...
def generate_service_sdk(service, sdk_language, session, prepare_for_runtime=False):
    ...
    template = Path(os.path.dirname(path), "..", "sdk", f"MrsServiceTemplate{fileExt}").read_text()
...
def substitute_schemas_in_template(service, template, sdk_language, session):
    ...
    for loop in schema_loops:
        ...
        for schema in schemas:
            ...
            class_name = service_class_name + lib.core.convert_path_to_pascal_case(schema.get("request_path"))
            mapping = {
                "schema_name": name,
            ...
            filled_temp += Template(schema_template_with_obj_filled).substitute(**mapping)
...
def substitute_objects_in_template(service, schema, template, sdk_language, session):
    ...
    service_class_name = lib.core.convert_path_to_pascal_case(service.get("url_context_root"))
    schema_class_name = lib.core.convert_path_to_pascal_case(schema.get("request_path"))
    ...
    for loop in object_loops:
        ...
        for db_obj in db_objs:
            ...
            # Get objects
            objects = lib.db_objects.get_objects(session, db_object_id=db_obj.get("id"))
            if len(objects) > 0:
                # Get fields
                ...
                for field in fields:
                    if (field.get("lev") == 1):
                        ...
                        if (field_is_pk(field)):
                            ...
                            obj_string_pk_list.append(f'String({name}.{field.get("name")})')
                            obj_string_args_where_pk_list.append(f'String(args.where.{field.get("name")})')
            
            obj_interfaces = generate_interfaces(fields, class_name, sdk_language)
            ...
            mapping = {
                ...
                "obj_full_request_path": 
                    service.get("url_context_root") + schema.get("request_path") + db_obj.get("request_path"),
            ...
            filled_temp += Template(schema_template).substitute(**mapping)
    ...
    if db_column_info:
        ...
        if (sdk_language == "TypeScript"):
            if (db_datatype == "tinyint(1)"):
                datatype = "boolean"
            if (db_datatype.startswith("tinyint") or
...
def generate_interfaces(fields, class_name, sdk_language):
    obj_interfaces = ""
    fields_listing = ""
    for field in fields:
        if (field.get("lev") == 1):
            ...
            if field.get("represents_reference_id"):
                ...
                obj_interfaces += generate_nested_interfaces(
                    ...
                    reference_class_name_postfix=lib.core.convert_path_to_pascal_case(field.get("name")),
