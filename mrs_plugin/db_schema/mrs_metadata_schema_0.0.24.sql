...
CREATE TABLE IF NOT EXISTS `mysql_rest_service_metadata`.`parameter` (
  ...
  `param_datatype` ENUM('STRING', 'INT', 'DOUBLE', 'BOOLEAN', 'LONG', 'TIMESTAMP', 'JSON') NOT NULL COMMENT 'The type of the parameter',
  `mode` ENUM("IN", "OUT", "INOUT") NULL,
...
CREATE TABLE IF NOT EXISTS `mysql_rest_service_metadata`.`auth_app` (
  ...
  `default_auth_role_id` INT UNSIGNED NULL COMMENT 'If set, a new user that has not any auth_roles assigned will get this role assigned when he logs in the first time.',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `mysql_rest_service_metadata`.`user_hierarchy` (
  `auth_user_id` INT UNSIGNED NOT NULL,
  `reporting_to_user_id` INT UNSIGNED NOT NULL,
  `user_hierarchy_type_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`auth_user_id`, `reporting_to_user_id`, `user_hierarchy_type_id`),
...
START TRANSACTION;
USE `mysql_rest_service_metadata`;
INSERT INTO `mysql_rest_service_metadata`.`auth_privilege` (`id`, `auth_role_id`, `crud_operations`, `service_id`, `db_schema_id`, `db_object_id`) VALUES (1, 1, 'CREATE,READ,UPDATE,DELETE', NULL, NULL, NULL);
