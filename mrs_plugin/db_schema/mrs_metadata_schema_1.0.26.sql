...
CREATE TABLE IF NOT EXISTS `mysql_rest_service_metadata`.`router_session` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_router_session_auth_user_idx` (`auth_user_id` ASC) VISIBLE,
  INDEX `fk_router_session_service_idx` (`service_id` ASC) VISIBLE,
  CONSTRAINT `fk_router_session_auth_user`
    FOREIGN KEY (`auth_user_id`)
    REFERENCES `mysql_rest_service_metadata`.`mrs_user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_router_session_service`
    FOREIGN KEY (`service_id`)
    REFERENCES `mysql_rest_service_metadata`.`service` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
DELIMITER $$
USE `mysql_rest_service_metadata`$$
CREATE FUNCTION `mysql_rest_service_metadata`.`get_sequence_id`() RETURNS BINARY(16) NOT DETERMINISTIC 
RETURN UUID_TO_BIN(UUID(), 1)$$
...
-- -----------------------------------------------------
USE `mysql_rest_service_metadata`;
CREATE  OR REPLACE SQL SECURITY INVOKER VIEW schema_version (major, minor, patch) AS SELECT 1, 0, 26;

-- -----------------------------------------------------
-- View `mysql_rest_service_metadata`.`mrs_user_schema_version`
-- -----------------------------------------------------
USE `mysql_rest_service_metadata`;
CREATE  OR REPLACE SQL SECURITY INVOKER VIEW mrs_user_schema_version (major, minor, patch) AS SELECT 1, 0, 26;
USE `mysql_rest_service_metadata`;

DELIMITER $$
USE `mysql_rest_service_metadata`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mysql_rest_service_metadata`.`auth_user_BEFORE_INSERT` BEFORE INSERT ON `auth_user` FOR EACH ROW
BEGIN
...
USE `mysql_rest_service_metadata`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mysql_rest_service_metadata`.`auth_user_BEFORE_UPDATE` BEFORE UPDATE ON `auth_user` FOR EACH ROW
BEGIN
    IF JSON_STORAGE_SIZE(NEW.app_options) > 16384 THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = "The JSON value stored in app_options must not be bigger than 16KB.";
    END IF;
...
GRANT SELECT, INSERT ON `mysql_rest_service_metadata`.`auth_user` 
	TO 'mrs_provider_metadata';
GRANT SELECT ON `mysql_rest_service_metadata`.`auth_user` 
...
GRANT SELECT, INSERT ON `mysql_rest_service_metadata`.`user_hierarchy` 
	TO 'mrs_provider_metadata';
GRANT SELECT ON `mysql_rest_service_metadata`.`user_hierarchy` 
	TO 'mrs_provider_data_access';

-- `mysql_rest_service_metadata`.`user_hierarchy_type`
GRANT SELECT, INSERT, UPDATE, DELETE
    ON `mysql_rest_service_metadata`.`user_hierarchy_type` 
    TO 'mrs_service_admin';
GRANT SELECT, INSERT ON `mysql_rest_service_metadata`.`user_hierarchy_type` 
	TO 'mrs_provider_metadata';
...
GRANT SELECT, INSERT ON `mysql_rest_service_metadata`.`user_group_hierarchy` 
	TO 'mrs_provider_metadata';
GRANT SELECT ON `mysql_rest_service_metadata`.`user_group_hierarchy` 
	TO 'mrs_provider_data_access';
...
GRANT EXECUTE ON PROCEDURE `mysql_rest_service_metadata`.`get_sequence_id`
